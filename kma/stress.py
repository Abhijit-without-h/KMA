"""Load / pressure test for the KMA retrieval pipeline.

Answers the operational questions:
  * how does single-query latency scale with corpus size N?
  * under CONCURRENT load (many queries at once), what throughput (q/s) and tail
    latency (p50/p95/p99) do we get?
  * how much does the corpus cost to build / hold (write pressure)?

Uses the hashing embedder so we measure PIPELINE overhead, not model inference
(with a real embedder, encode() dominates and the same caching wins apply). Run:

    python -m kma.stress
    python -m kma.stress --sizes 500,2000,10000 --concurrency 1,8,32 --mode ensemble
"""
...
_WORDS = ("alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu "
          ...
          "capital language safety systems celebrity landmark tower mission planet").split()
...
def run_load(mem: AgenticMemory, queries: list[str], *, mode: str,
    ...
    return {"qps": total / wall, "p50": p50, "p95": p95, "p99": p99,
...
def main():
    ...
    print(f"\nKMA load test   mode={args.mode}   {args.total} queries/run "
          f"(hashing embedder)")
    print("=" * 76)
    for n in sizes:
        mem, build_s, queries = build_memory(n)
        mem.warmup()                       # pay the one-time index build up front
        print(f"\nN={n:,}   built in {build_s:.2f}s "
              f"({n/build_s:,.0f} writes/s)")
        print(f"  {'concurrency':>11} {'throughput':>12} {'p50':>9} "
              f"{'p95':>9} {'p99':>9}")
        for c in concs:
            r = run_load(mem, queries, mode=args.mode, concurrency=c,
                         total=args.total)
            print(f"  {c:>11} {r['qps']:>9,.0f} q/s {r['p50']:>7.2f}ms "
                  f"{r['p95']:>7.2f}ms {r['p99']:>7.2f}ms")
        batch_qps = run_batch(mem, queries, args.total)
        print(f"  {'batch':>11} {batch_qps:>9,.0f} q/s  (shared snapshot + "
              f"one batched embed)")
